...
# Волна улучшений UX/UI/Perf/SEO/A11y — 2026-07-16

Полный аудит по PD «Полный аудит и улучшение сайта». Коммиты `630fa50…fef5b2a`.

## Производительность

| Метрика | До | После |
|---|---|---|
| Главный JS-чанк | **1 923 KB** (gzip 540 KB) | **448 KB** |
| JS-чанков | 1 | 117 (lazy по маршрутам) |
| recharts / KaTeX | в главном чанке | отдельные чанки (362/256 KB), грузятся по требованию |
| Время сборки | 56 c | 9 c |
| Шрифты | render-blocking `@import` в CSS | `preconnect` + `<link>` в index.html |

## Сделано

- **Code splitting**: все страницы кроме главной — `React.lazy` + Suspense
  с доступным PageLoader (role=status, скелет шапки).
- **ScrollToTop**: прокрутка вверх при переходах (был сохраняющийся скролл),
  поддержка hash-якорей — кнопка «Обзор модулей» (`/#modules`) заработала.
- **SEO**: per-page `title`/`description` на 85+ страницах через SEOHead в
  layout-обёртках (TopicPageLayout, LessonLayout, FunctionPageLayout,
  SectionHub, гид, визуализации, 404). Future-флаги роутера убрали
  предупреждения консоли.
- **A11y**: skip-link + `main#main-content`; `prefers-reduced-motion`;
  aria-expanded/русские aria-label в шапке; focus-ring у дропдауна;
  хлебные крошки уроков — `<nav>/<ol>` + `Link` вместо `span onClick`;
  двойной `h1` на странице вероятностей понижен до `h2`;
  `aria-pressed` на переключателях «Теория/Задачи».
- **UX-тупики устранены**: 404 переведена на русский, получила шапку и
  подборку разделов (был англ. текст без навигации и `console.error`);
  страницы визуализаций обёрнуты в Layout (не имели никакой навигации);
  «Модуль не найден» в гиде получил кнопку на главную; пустой модуль гида
  показывает «Модуль в разработке» вместо «Экран undefined» и «1 / 0».
- **Единый дизайн**: один компонент Footer вместо двух разных футеров
  (inline-копия в Layout + компонент), футер расширен навигацией по разделам.
- **Код**: удалён неиспользуемый `NavLink.tsx`; компоненты `Index` в
  visualizations переименованы по имени файла; включён HMR overlay.

## Рекомендации дальше

1. OG-изображение в `/public` (og:image/twitter:card удалены за неимением).
2. Vite 8 отдельным PR — закроет 2 оставшиеся audit-уязвимости.
3. Тёмная тема: токены `.dark` есть, но переключателя в UI нет
   (`next-themes` установлен) — быстрый выигрыш.
4. Браузерный прогон интерактивных графиков (асимптоты, точки склейки,
   Риман, фазовые портреты) — вручную или Playwright.
5. Данные `courseData.isUnlocked/progress` статичны — связать с реальным
   прогрессом из `localStorage` (lib/progress.ts).
